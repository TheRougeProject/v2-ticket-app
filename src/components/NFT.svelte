<script>
  ...
  import { provider, signer, signerAddress, chainId, chainData } from 'svelte-ethers-store'
  ...
  } from '@rouge/contracts/rouge'

  import Rouge from '@rouge/contracts/Rouge.json'

  import blockchain from '$lib/blockchain.js'
  import { formatTextMaxLength } from '$lib/utils'
...
<Modal bind:this={modal} bind:active={qrActive} noCloseButton={true}>
  <div class="modal-card">
        ...
        <div class="level-left">
          ...
          <div class="level-item">
            <div class="">
              ...
              <p class="heading mt-2">NFT {nft.redeemed ? 'Ticket Stub' : 'Ticket'} tokenId #{tokenId}</p> <!-- todo link explorer -->
            </div>
          </div>
        </div>
        <div class="level-right">
          <div class="level-item ">
            <button class="delete" aria-label="close" on:click={() => {modal.close(); isUnlocked = false}}></button>
          </div>
        </div>
      </div>
    </header>
    <section class="modal-card-body" style="position: relative; min-height: 432px;">
      {#if !nft.redeemed}
        <p class="has-text-centered field">
          {#if !qrUnlocked}
            <a class="button is-primary mr-3" on:click={certify}>Sign a ticket proof</a>
          {:else}
            <input id="unlock" type="checkbox" class="switch" disabled={qrUnlocked ? undefined : true } bind:checked={isUnlocked}>
            <label for="unlock">{isUnlocked ? 'Lock ticket QR to keep your proof safe' : 'Unlock ticket QR to enable check-in proof scan'}</label>
          {/if}
        </p>
      {/if}
      {#if isUnlocked}
        <article class="message is-warning">
          <div class="message-body">
            This is your unlocked ticket QR that you can use for check-in proof scan. Don't share it with untrusted parties!
          </div>
        </article>
        <QR color="#900" text={qrUnlocked} bind:src={src}/>
      {:else}
        {#if nft.redeemed}
          <article class="message is-info">
            <div class="message-body">
              Ticket Stub : this NFT has alredy been used but you can still share the ticket identifier.
            </div>
          </article>
        {:else}
          <article class="message is-info">
            <div class="message-body">
              This QR code is a NFT ticket identifier that you can share safely without any risk.
              {#if qrUnlocked}
                You can unlock this ticket to display the ticket proof that is used for check-in.
              {:else}
                This NFT ticket was created on another device or browser, so you must sign a certificate for this ticket (this operation is gas free) to build a valid proof usable during the check-in process.
              {/if}
            </div>
          </article>
        {/if}
        <QR text={did} />
      {/if}
      <p class="has-text-centered is-size-7"><small><span>{did}</span></small></p>
    </section>
    {#if isUnlocked}
    <footer class="modal-card-foot">
      ...
      -->
      {#if dev}
      <button class="button" on:click={() => saveLocal(qrData)}>devpush {proofType}</button>
      {/if}
    </footer>
    {/if}
  </div>
...
<div class="card mb-2">
  ...
  <div class="card-content p-0">
    <div class="media" on:click={openQR}>
      ...
      <div class="media-content is-size-7">
        <p class="icon-text">
            ...
            <Icon name="qrcode" class="pt-1 ml-2 is-small {proofType ? 'is-proven' : ''}" />
        </p>
        ...
        {/if}
          {#if channel.icon}
              ...
              <figure class="image is-24x24"><img data-ipfs={channel.icon} use:ipfs /></figure>
          {/if}
