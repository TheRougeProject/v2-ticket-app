...
<div class="field" >
  ...
  {:else}
  <button disabled={(disabled || call.step) ? true : false} class={importedClasses} on:click={submit}>
    <slot />
  </button>
  {/if}
  <p class="help">
    ...
    <span class="icon-text is-warning">
      <span>{check.errMsg || 'checks failed'}</span> <Icon name="thumbs-down" size="14" />
    </span>
    {:else if call.err}
    <span class="icon-text is-danger">
      <span>{call.errMsg || 'tx failed'}</span> <Icon name="thumbs-down" size="14" />
    </span>
    {:else if call.step === 1}
    <span class="icon-text">
      </span><span>waiting wallet</span><span class="icon"><span class="loader"></span>
    </span>
    {:else if call.step === 2}
    <span class="icon-text">
      <span class="icon mr-0"><span class="pending"></span></span>
      <a target="_blank" href={explorer($chainData,'tx',call.tx.hash)}>pending</a>
      <Icon name="external-link" size="14" />
    </span>
    {:else if call.step === 3}
    <span class="icon-text">
      <Icon name="check" size="14" class="mr-0" />
      <a target="_blank" href={explorer($chainData,'tx',call.tx.hash)}>success</a>
      <Icon name="external-link" size="14" />
    </span>
    {/if}
  </p>
