<script>
  ...
  onMount( () => {
    ...
    if (value && value[0]) {
      ...
      startTime = `${new Date(value[0]).getHours()}:${new Date(value[0]).getMinutes()}`
    }
    if (value && value[1]) {
      endDate = new Date(value[1])
      endTime = `${new Date(value[1]).getHours()}:${new Date(value[1]).getMinutes()}`
    }
...
<div class={importedClasses} importedClasses bind:this={range} type="vdate">x</div>
