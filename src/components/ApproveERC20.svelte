<script>
  ...
  const load = async () => {
    balance = await blockchain.erc20(amount.token.address).balanceOf($signerAddress)
    allowance = await blockchain.erc20(amount.token.address).allowance(
      $signerAddress, contract
    )
    console.log(
      '[%s] balance = %s / allowance = %s ',
      amount.token.symbol, utils.formatUnits(balance+'', amount.token.decimals), utils.formatUnits(allowance+'', amount.token.decimals)
    )
...
{#if !allowed}
<TxButton disabled={state ? undefined : true} class="button" submitCtx={approveCtx} on:success={load}>
  Approve {amount+''}
</TxButton>
{/if}
