...
import { defaultEvmStores as evm, provider, signer, signerAddress, chainId } from 'svelte-ethers-store'
...
const createStore = () => {
  ...
  const bearerKey = ($chainId, $signerAddress) => `rge:bearer:${$chainId || evm.$chainId}:${$signerAddress || evm.$signerAddress}`
  ...
  const readEVMBaseState = async address => {
    ...
    meta.balances = { }
    const enabled = await decodeRoles(blockchain.rouge(address), [ 'acquire', 'redeem' ], constants.AddressZero, meta.channels)
    meta.state = { acquired: 0, redeemed: 0, enabled: enabled[channels.length] }

    const TOKENS = getChainTokens(evm.$chainId, evm.$chainData)
    meta.channels = await Promise.all(channels.map(({ supply, totalAcquired, totalRedeemed, amount, token }, i) => {
      ...
      if (meta.channels[i].amount?.token?.symbol === 'USDC') {
        meta.channels[i].amount.token = { ...meta.channels[i].amount.token, ...TOKENS.USDC }
      }
      if (meta.channels[i].amount?.token?.symbol === 'ETH') {
        meta.channels[i].amount.token = { ...meta.channels[i].amount.token, _isNative: true }
      }
      ...
      return { ...meta.channels[i], _state: { supply, totalAcquired, totalRedeemed, enabled: enabled[i] } }
    }))
    ...
    // XXX remove obsolete
    // if (/^http/.test(meta.visual)) {
    //   meta._visualSrc = meta.visual
    // } else if (meta.visual) {
    //   meta._visualSrc = await ipfs.getImageURL(
    //     /^ipfs/.test(meta.visual) ? meta.visual : 'ipfs://' + meta.visual
    //   )
    // }
    assign({
      [address]: { tags: { event: 'x' }, uri,  ...meta, _chainId: evm.$chainId, _address: address, _loaded: true }
    })
    emit()
  }
  ...
  const refresh = async address => {
    ...
    try {
      ...
      if (/^0x0000000000/.test(address)) {
        const data = { ...JSON.parse(localStorage.getItem(`rge:draft:${address}`)), _chainId: evm.$chainId, _isDraft: true, _loaded: true }
  ...
  const init = () => {
    draftNonce = browser ? (parseInt(localStorage.getItem(draftNonceKey())) || 1) : 1
