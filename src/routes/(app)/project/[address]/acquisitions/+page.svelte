<script>
  ...
  const filter0 = (params, filterType) => tokenId => {
    if (filterType === 'id' && filterParams.id && tokenId !== filterParams.id) return false
    ...
    if (filterType === 'channel' && filterParams.label != null) {
      ...
      if ($nft[`${address}:${tokenId}`].channelId !== filterParams.label) return false
    }
    return true
  }
...
<div class="tabs is-boxed is-toggle" class:is-active={!!selected} class:is-on={!!filterType}>
  <ul>
    <li class:is-active={selected === 'filter'}>
      <a on:click={() => {selected = selected === 'filter' ? undefined : 'filter'}}>
    ...
    <li class:is-active={selected === 'group'}>
      <a on:click={() => {selected = selected === 'group' ? undefined : 'group'}}>
...
<div class="box filter-box" class:is-active={!!selected}>
 ...
 <div class="columns is-multiline" >
  ...
  <div class="column is-one-fifth">
    <div class="field" >
      <label class="label"><input bind:group={filterType} class="mr-1" type="radio" name="filter" value="id">by id</label>
      <p class="control">
        <input class="input" on:change={() => {filterType = 'id'}} class:is-danger={control.error.label} type="number" min="1"  max={nfts.length}  bind:value={filterParams.id}>
      </p>
    </div>
  </div>
  ...
  <div class="column is-one-third">
    <div class="field" >
      <label class="label"><input bind:group={filterType} class="mr-1" type="radio" name="filter" value="address">by address</label>
      <p class="control">
        <input class="input" on:change={() => {filterType = 'address'}} class:is-danger={control.error.label} type="text" bind:value={filterParams.address}>
      </p>
    </div>
  </div>
  ...
  <div class="column is-one-third">
    <div class="field" >
      <label class="label"><input bind:group={filterType} class="mr-1" type="radio" name="filter" value="channel">by channel</label>
      <p class="control">
        <span class="select" on:change={() => {filterType = 'channel'}} >
