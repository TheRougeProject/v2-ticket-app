...
{:else}
  ...
  {#each Object.keys(p.balances||{}) as token}
    ...
    <Slate>
      <p>{p.balances[token].token.symbol}
        {#if token === constants.AddressZero}
          <Address noIdenticon={true } address={address}>native coin</Address>
        {:else}
      ...
      <div slot="actions">
        ...
        <TxButton class="button is-outlined is-primary" disabled={p.balances[token].number.eq(0)}  submitCtx={() => withdrawCtx(token)}>
          Withdraw All
        </TxButton>
