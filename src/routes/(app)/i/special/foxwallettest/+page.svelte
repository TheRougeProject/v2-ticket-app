<script>
  ...
  const chain = 42170
  const address = '0xae6DDf4b8782Ba08d89FfbA42f5d7A9bB3BdBBa3'

  $: p = $project[address] || {}

  $: validChain = $chainId === chain

  $: hash = $signerAddress ? utils.id($signerAddress) : utils.id(constants.AddressZero)

  $: nfts = $nft.keys ? $nft.keys.filter(key => key.split(':')[0] === address && $nft[key].owner === $signerAddress) : []

  $: balances = $signerAddress ? $account.balancesFor(address) : {}
  $: checksIn = $signerAddress ? $account.checksIn(address) : {}
...
<section class="sectionx">
  ...
  <h2 class="title">Test only - Not playable</h2>
  <h3 class="subtitle is-size-6 mb-5">Join us <a target="_blank" href="https://discord.gg/aUeSjsN8Tx">on Discord</a> for rules and help!</h3>
  ...
  {#if isWinner}
    <p>
      Well done! You can win the big 500&nbsp;DAI token big prize!
      <br />
      You can also earn points by claiming all your winning redeemed Bingo in the Rouge community program.
      (see details <a target="_blank" href="https://discord.gg/aUeSjsN8Tx">in our discord channel #FoxWallet-Bingo</a>)
    </p>
  {/if}
