<script>
  ...
  import { TokenAmountSet } from '@rouge/contracts/Token'
  ...
  $: isOur = $signerAddress && $project.addresses && $project.addresses.includes(contract) || false
  ...
  const buyCtx = async () => {
    ...
    Object.keys($cart).forEach(channelId => {
      ...
      // last test over max
      if (p.channels[channelId].max && $cart[channelId] > p.channels[channelId].max - (balances[channelId] || 0)) {
        $cart[channelId] = p.channels[channelId].max - (balances[channelId] || 0)
        step = 1
        return
      }
...
<Modal bind:this={modal} active={success} on:close noCloseButton={true}>
  <div class="modal-card">
    <section class="modal-card-body has-background-light has-text-centered">
      ...
      <p class="my-4"><a class="my-4 is-primary" href="/book">See them in the tickets book or as NFT in your wallet...</a></p>
    ...
    <nav class="level is-mobile mb-2">
      <div class="level-left">
        {#if !p._isDraft}
          <div class="level-item">
            {#if /0xaFbe5Ea6b281ddCbb72273953293fE4A852Dbc00/i.test(contract)}
              <a class="button is-text is-small is-responsive" href="/i/special/foxwallet">
                <span>Back to Bingo Game</span>
              </a>
            {:else if balances.total}
              <a class="button is-text is-small is-responsive" href="/book">
                <Icon name="Book" class="mr-1" />{balances.total} ticket{balances.total > 1 ? 's' : ''} already collected
              </a>
            ...
            {:else if p._loaded}
              <a class="button is-text is-small is-responsive" on:click={bookmark}>
      ...
      {#if isOur}
        <div class="level-right">
          <div class="level-item">
            <a class="button is-text is-small is-responsive" href="/project/{contract}/{ $project[contract]._isDraft ? 'draft/' : ''}">
      ...
      <Project {p}>
          ...
          {:else}
            ...
            <table class="table is-striped is-fullwidth is-hoverable">
              ...
              <tbody>
                {#each (p.channels||[]) as channel, i}
                  <tr style="vertical-align: center;">
                    ...
                    {/if}
                    <th colspan={channel.icon ? 4 : 5} style="vertical-align: center;">{channel.label}</th>
                    ...
                    <td colspan="1" class="has-text-right">
                      <input
                        ...
                        max={channel.max - (balances[i] ||0) || undefined}
                               disabled={channel.max && (balances[i] ||0) >= channel.max ? true : undefined}
                        bind:value={$cart[i]}
                      >
                      {#if channel.max && balances[i] >= channel.max}
                        <p class="help is-info">maximum {channel.max} reached</p>
                      {:else if channel.max}
                        <p class="help is-info">maximum = {channel.max}{#if balances[i]}, collected = {balances[i] || 0}{/if}</p>
                      {/if}
                    </td>
                  </tr>
                {/each}
              ...
              <thead>
                <tr>
                  <th colspan="5"></th>
                  <th colspan="3" class="has-text-right">Total = {$cart.total || 'tx gas only'}</th>
                </tr>
              </thead>
              <tfoot >
                <tr>
        ...
        {#if step >= 3 && validChain}
              ...
              <div class="box level slate">
                <div class="level-left">
                  {#if p.channels[entry].icon}
                    <div class="level-item">
                      <figure class="image is-48x48">
                        <img data-ipfs={p.channels[entry].icon} use:ipfs />
                      </figure>
                    </div>
                  {/if}
                  <div class="level-item">
          ...
          <div class="box m-6 has-text-right">
            ...
            {#if !success}
              <TxButton disabled={!ready || p._isDraft} class="button is-primary mt-4" submitCtx={buyCtx} on:success={() => success = true}>
                Checkout {#if $cart.total}(total={$cart.total}){/if}
              </TxButton>
              <p>After payment, each purchased ticket will be delivered as an NFT.</p>
            {/if}
