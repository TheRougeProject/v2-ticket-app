<script>
  ...
  $: nfts = $nft.keys ? $nft.keys.filter(key => key.split(':')[0] === contract && $nft[key].owner === $signerAddress) : []
...
<Project {p}>
  ...
  <EmptyState svg="/bad-connection.svg">
    {#if !validChain}
    <h3 class="subtitle"><strong>Not connected to {chainName || `chainId ${chain}`}</strong></h3>
    {:else}
    ...
    <p class="help is-info">Connect your wallet to {chainName || `chainId ${chain}`} to check your tickets for this event</p>
  </EmptyState>
